...
function _ensure_schema(PDO $pdo): void {
  ...
  try{
    $done = $pdo->prepare("SELECT 1 FROM app_settings WHERE `key`=? LIMIT 1");
    $done->execute(['migration_2026_06_16_yearly_semester2_to_year_done']);
    if(!$done->fetchColumn()){
      $pdo->exec("INSERT INTO events(type, actor_user_id, created_at, payload_json)
                  SELECT
      ...
      $pdo->exec("INSERT INTO final_assessments(
      ...
      $st = $pdo->prepare("INSERT IGNORE INTO app_settings(`key`,`value`,updated_at,created_at) VALUES(?,?,NOW(),NOW())");
      $st->execute(['migration_2026_06_16_yearly_semester2_to_year_done', date('Y-m-d H:i:s')]);
    }
  }catch(Exception $e){
